...
title '1 项目压缩'
tar -czf $project_name.tar.gz --exclude='node_modules' --exclude='.next' --exclude='package-lock.json' -C $current_dir .
title '2 创建远程目录'
...
title '6 执行远程部署脚本'
ssh $user@$ip "export time=$time; /usr/bin/zsh $deploy_dir/setup_for_remote.sh"
